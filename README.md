...
- **フロント**：React 18 + Vite + Tailwind、Three.js r128（`frontend/`）
- **バックエンド**：Supabase（Auth / PostgreSQL / Edge Functions）
- **インフラ**：Vercel（`frontend` ルート）、GitHub Actions（CI / PR Preview）
- **利用 AI ツール**：Google Gemini API（クローン会話・1日シミュレーション）、Cursor 等（開発）

### 使用した外部 API / サービス

| サービス名 | 用途 | プラン | 備考 |
|---|---|---|---|
| Supabase | DB / Auth / Edge Functions | ハッカソン提供想定 | `VITE_*` は Vercel、サービスロールは Edge のみ |
| Google Gemini API | クローンチャット・Topic 生成・1日シミュレーション | API キー要 | `GEMINI_API_KEY`（Supabase Secret） |
| Vercel | フロント Hosting / Preview | チームプロジェクト | Root Directory = `frontend` |
